...
6. Проверить checkbox off/on, rerun без API, Approve/Reject, изменение после Approve,
   ...
   живой вызов через UI также проверен на SYNTHETIC/TRAINING до `pending_approval`.
...
заявляется. UI-интеграция проверена через AppTest и живой запрос на синтетическом
примере; точность произвольных ответов модели этим не доказана. Встроенный табличный экспорт
