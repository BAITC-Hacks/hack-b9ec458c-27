...
Реализован в ветке `agent-integration`. В `main` автоматически не объединяется.
Текущий `app.py` пока не вызывает агента. Используется ограниченный цикл function
...
6. Проверить checkbox off/on, rerun без API, Approve/Reject, изменение после Approve,
   ошибку API и недоступные данные. Live UI-проверки агентной части ещё не выполнены.
...
запуска IDE либо согласованный загрузчик при UI-интеграции. Секреты не коммитить.
...
заявляется. Следующий шаг: UI-интеграция и выбранная живая модель на синтетическом примере.
